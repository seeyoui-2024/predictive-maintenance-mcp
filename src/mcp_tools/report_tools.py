...
async def generate_diagnostic_report_docx(
    ...
    result = save_diagnostic_report_docx(signal_id, sections, title=title, language=language)
...
async def plot_signal(
    ...
    # Metadata
    metadata = {
        "report_type": "plot_signal",
        "signal_id": signal_id,
        "duration": float(duration),
        "num_samples": int(len(signal_plot)),
        "sampling_rate": float(sampling_rate),
        "show_statistics": show_statistics,
    }

    # Generate HTML using template
    from .html_templates import create_plot_signal_report
    html = create_plot_signal_report(
        signal_id=signal_id,
        plot_div=plot_div,
        duration=float(duration),
        num_samples=int(len(signal_plot)),
...
async def generate_iso_report(
    ...
    # Generate and save report (signal_id is the report's signal label)
    result = save_iso_report(signal_file=signal_id, iso_result=iso_dict, language=language)
...
async def generate_pca_visualization_report(
    ...
    **IMPORTANT**: Labels show MODEL PREDICTIONS, not ground truth. Use `true_labels`
    ...
    # Generate HTML using template
    from .html_templates import create_pca_visualization_report
    html = create_pca_visualization_report(
        model_name=model_name,
        plot_div=plot_div,
        metadata=metadata,
        summary=summary,
...
async def generate_feature_comparison_report(
    ...
    **Strategy**: Same HTML report approach as other reports. Useful for understanding
    ...
    # Generate HTML using template
    from .html_templates import create_feature_comparison_report
    group_names_str = " vs ".join(signal_groups.keys())
    html = create_feature_comparison_report(
...
async def generate_diagnostic_report(
    ...
    the standards caveat that travels with every severity verdict must not
    be dropped or paraphrased. If a question is not answered by these
    ...
    saved = save_integrated_diagnostic_report(advisory, figure, formats=requested, language=language)
    files = saved["files"]
