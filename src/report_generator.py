...
def save_fft_report(
    ...
    return {
        ...
        "metadata": metadata,
        "message": f"✓ {i18n.t('fft_report_saved')}: {output_file.name} ({output_file.stat().st_size / 1024:.1f} KB)",
    }
...
def save_envelope_report(
    ...
    return {
        ...
        "metadata": metadata,
        "message": f"✓ {i18n.t('envelope_report_saved')}: {output_file.name} ({output_file.stat().st_size / 1024:.1f} KB)",
    }


def save_iso_report(signal_file: str, iso_result: Dict[str, Any], language: str = "en") -> Dict[str, Any]:
    """
    ...
    html = create_iso_report(
        signal_file=signal_file, iso_result=iso_result, metadata=metadata, language=language
    )
    ...
    return {
        ...
        "rms_velocity": iso_result["rms_velocity"],
        "metadata": metadata,
        "message": f"✓ {i18n.t('iso_report_saved')}: {output_file.name} - {i18n.t('zone_' + iso_result['zone'].lower())} ({iso_result['severity_level']})",
    }
...
def save_integrated_diagnostic_report(
    ...
    html = create_integrated_diagnostic_report(
        advisory, figure, generated_at=generated_at, language=language
    )
...
def save_diagnostic_report_docx(
    ...
    # -- title ------------------------------------------------------------
    heading = title or f"{i18n.t('diagnostic_report_docx')} — {signal_file}"
    doc.add_heading(heading, level=0)
    doc.add_paragraph(
        f"{i18n.t('generated')}: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}  •  "
        f"{i18n.t('signal')}: {signal_file}"
    )

    # -- 1. Statistics table ----------------------------------------------
    stats = sections.get("statistics")
    if stats and isinstance(stats, dict):
        doc.add_heading(i18n.t("statistical_summary"), level=1)
        table = doc.add_table(rows=1, cols=2, style="Light Shading Accent 1")
        table.alignment = WD_TABLE_ALIGNMENT.CENTER
        hdr = table.rows[0].cells
        hdr[0].text, hdr[1].text = i18n.t("parameter"), i18n.t("value")
    ...
    if fft_peaks:
        doc.add_heading(i18n.t("fft_spectrum_top_peaks"), level=1)
    ...
    if env_peaks:
        doc.add_heading(i18n.t("envelope_analysis_top_peaks"), level=1)
        table = doc.add_table(rows=1, cols=3, style="Light Shading Accent 1")
        table.alignment = WD_TABLE_ALIGNMENT.CENTER
        hdr = table.rows[0].cells
        hdr[0].text, hdr[1].text, hdr[2].text = (
    ...
    if bf and isinstance(bf, dict):
        doc.add_heading(i18n.t("bearing_characteristic_frequencies"), level=1)
        table = doc.add_table(rows=1, cols=2, style="Light Shading Accent 1")
        table.alignment = WD_TABLE_ALIGNMENT.CENTER
        hdr = table.rows[0].cells
        hdr[0].text, hdr[1].text = i18n.t("frequency_label"), i18n.t("value_hz")
    ...
    if iso and isinstance(iso, dict):
        doc.add_heading(i18n.t("iso_20816_evaluation"), level=1)
        zone = iso.get("zone", "?")
        severity = iso.get("severity_level", "?")
        rms_v = iso.get("rms_velocity", 0)
        p = doc.add_paragraph()
        p.add_run(f"{i18n.t('evaluation_zone')} {zone}").bold = True
        p.add_run(f"  —  {i18n.t(severity.lower()) if i18n.t(severity.lower()) != severity.lower() else severity}  ({i18n.t('rms_velocity')}: {rms_v:.3f} {i18n.t('mm_s')})")
        if iso.get("thresholds"):
            doc.add_paragraph(f"{i18n.t('thresholds')}: {iso['thresholds']}")

    # -- 6. Free-text diagnosis -------------------------------------------
    diagnosis = sections.get("diagnosis")
    if diagnosis:
        doc.add_heading(i18n.t("diagnostic_summary"), level=1)
        # Translate diagnosis text if possible
        diagnosis_text = str(diagnosis)
        if i18n:
            # Replace common English terms with translations
            diagnosis_text = diagnosis_text.replace("Machine in Zone", i18n.t("machine_condition") + " - " + i18n.t("zone"))
            diagnosis_text = diagnosis_text.replace("zone", i18n.t("zone"))
            diagnosis_text = diagnosis_text.replace("Zone", i18n.t("zone"))
            for zone in ["A", "B", "C", "D"]:
                diagnosis_text = diagnosis_text.replace(f"zone {zone}", i18n.t(f"zone_{zone.lower()}"))
                diagnosis_text = diagnosis_text.replace(f"Zone {zone}", i18n.t(f"zone_{zone.lower()}"))
            for severity in ["New", "Acceptable", "Unsatisfactory", "Severe"]:
                diagnosis_text = diagnosis_text.replace(severity, i18n.t(severity.lower()))
    ...
    return {
        ...
        "message": f"✓ {i18n.t('docx_report_saved')}: {output_file.name} ({output_file.stat().st_size / 1024:.1f} KB)",
    }
